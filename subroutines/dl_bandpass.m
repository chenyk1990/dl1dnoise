function [dout]=dl_bandpass(din,dt,flo,fhi,nplo,nphi,phase,verb)
% dl_bandpass: Bandpass filtering.
%
% Aug, 05, 2020
% by Yangkang Chen
%
...
fact=dl_ifnot(low,sin(0.5*arg),cos(0.5*arg));
fact=fact*fact;
...
if mod(nn,2)>0
    ...
    for ix=0:nx-1
        x1=x0;x0=x(ix+1);
        y0=dl_ifnot(bw.low,(x0 + x1 - d2 * y1)*d0,(x0 - x1 - d2 * y1)*d0);
        x(ix+1)=y0;
        y1=y0;
    end 
end
...
for j=0:floor(nn/2)-1
    ...
    for ix=0:nx-1
        ...
        y0=dl_ifnot(bw.low,(x0 + 2*x1 + x2 - d1 * y1 - d2 * y2)*d0,(x0 - 2*x1 + x2 - d1 * y1 - d2 * y2)*d0);
